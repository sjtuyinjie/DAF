...
class RGBBlock(nn.Module):
    def __init__(self, latent_dim, input_channel, upsample, rgba=False, override=True):
        super().__init__()
        self.input_channel = input_channel
        self.to_style = nn.Linear(latent_dim, 1)
...
class GeneratorBlock(nn.Module):
    ...
    def forward(self, x, prev_rgb, istyle, inoise):
        ...
        # print(inoise.shape, "cropped noise", self.to_noise1(inoise).shape, "expanded noise")
        noise1 = self.to_noise1(inoise).permute((0, 3, 1, 2))
        noise2 = self.to_noise2(inoise).permute((0, 3, 1, 2))

        style1 = self.to_style1(istyle)
        x = self.conv1(x, style1)
        # print(noise1.shape, x.shape, "shapes", self.to_noise1(inoise).permute(0,3,1,2).shape)
        # exit()
...
class Generator(nn.Module):
    def __init__(self, image_size, latent_dim, network_capacity=16, transparent=False, attn_layers=[], no_const=False,
        ...
        self.initial_override = nn.Parameter(torch.randn((1, init_channels, 1, 4)))
    ...
    def forward(self, styles, input_noise):
        ...
        # print(x.shape, "pre conv")
        # x = self.initial_conv(x)
        x = self.initial_override.expand(batch_size, -1, -1, -1)
        # print(x.shape, "post conv", self.initial_override.shape, x2.shape)
        # exit()

        # x = self.initial_override + 0.0
        # print(input_noise.shape, "NOISE")
        # exit()
...
class Trainer():
    ...
    def train(self):
        ...
        for i in gradient_accumulate_contexts(self.gradient_accumulate_every, self.is_ddp, ddps=[D_aug, S, G]):
            ...
            w_styles = styles_def_to_tensor(w_space)
            print(w_styles.shape, noise.shape)
