...
class soundsamples(torch.utils.data.Dataset):
    def __init__(self, arg_stuff):
        with open("/data/vision/torralba/scratch/chuang/Projects/nap_vision/all_container.pkl", "rb") as fff:
            metadata = pickle.load(fff)
        # all_points = []
        # for i in metadata.keys():
        #     all_points.append(metadata[i]["perception_pos"])
        # all_points = np.array(all_points)
        # plt.scatter(all_points[:,0], all_points[:,2])
        ...
        self.sound_data = []
        self.full_path = "//data/vision/torralba/scratch/chuang/Projects/yinjie/spectral_data_com.h5"
        self.sound_data = h5py.File(self.full_path, 'r')
        #new stft
        self.psd_path="/data/vision/torralba/scratch/chuang/Projects/yinjie/spectral_data_psd.h5"
...
# -43045.72863281244
        ...
        training_buffer = []
        fm = open('/data/vision/torralba/scratch/chuang/Projects/nap_vision/data_loading/removelist.txt', 'r')
        line = fm.readline()
        removelist = []
        while line:
        ...
        #exit()
        print("org len:",len(self.training))
        for k in self.training:
            # if self.metadata[k]["name_num"] == 1:

            # if "kitchen" in k and not (k in removelist):
