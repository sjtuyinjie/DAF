...
def test_net(rank, other_args):
    ...
    #weights = torch.load("//media/car/YJ06/amitprojects/projects/nap101/results/apartment_1/01990.chkpt", map_location=map_location)
    weights = torch.load("//home/zdp21n5/projects/aresnettest/results/apartment_1_psd_report///00980.chkpt",
    ...
    with torch.no_grad():
        ...
        for IDX in range(1016):
            ...
            losses = []
            pos = []
            data_stuff = dataset.getitem_testing(IDX)#getitem_whole_v3 getitem_testing
            #print(data_stuff)
            ...
            # exit() ctrl + shift + R
            gt = (data_stuff["training_data"]).to(output_device, non_blocking=True)[None]
            #print(gt)
            ...
            # print(material_pred.shape, type_pred.shape, residual_pred.shape, position_pred.shape)
            # exit()
            ...
            # print(position_pred.shape, "position prediction shape")
            # exit()
            ...
            # recovered_output_np = recovered_output.cpu().numpy()
            # print(recovered_output_np.min(), recovered_output_np.max(), gt[0].cpu().numpy().min(), gt[0].cpu().numpy().max())
            # plt.plot(recovered_output_np[0],c="orange")
            # plt.plot(gt[0].cpu().numpy(), c="blue")
            # # plt.plot()
            # plt.show()
            # exit()
            ...
            sum=sum+np.sqrt(np.square(position_pred0[0][0]-position_gt0[0][0])+np.square(position_pred0[0][1]-position_gt0[0][1]))
            if(np.sqrt(np.square(position_pred0[0][0]-position_gt0[0][0])+np.square(position_pred0[0][1]-position_gt0[0][1]))<2):
                sum2+=1
            if(np.sqrt(np.square(position_pred0[0][0]-position_gt0[0][0])+np.square(position_pred0[0][1]-position_gt0[0][1]))<5):
                sum3+=1
            if(IDX>0):
                print("error:",sum/IDX)
            ...
            for x_offset in np.arange(-5,5, 0.25):
                for y_offset in np.arange(-5, 5, 0.25):
                    ...
                    recovered_output = decoder(total_latent)
                    # print('output:')
                    # print(recovered_output)# not the same
                    ...
                    #print('gt:')
                    #print(position_gt)#42  all the same
                    #print('loss:')  #no problem
                    #print(reconstruction_criterion(recovered_output, gt).item())
            # print("losses shape:")
            # print(len(losses))  #list  1600  1
            # print('pos shape:')
            # print(len(pos))#l(ist  1600   2
            # print('position gt shape:')
            # print(position_gt.shape)  #torch.Size([40])
            ...
            print("sum2",sum2)
            print("sum3",sum3)
        with open("psd_report.pkl", "wb") as f:
