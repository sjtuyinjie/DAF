...
def test_net(rank, other_args):
    ...
    weights = torch.load("//data/vision/torralba/scratch/chuang/Projects/nap_vision/results/new/01480.chkpt",
    ...
    with torch.no_grad():
        ...
        for IDX in range(1016):  # 1016
            ...
            # our_type=type_pred.cpu().numpy()
            # bestcate=np.argmax(our_type)
            # if(bestcate==type_num_gt.cpu().numpy())
            #     type_ok+=1
            # exit(-1)
            # gt type: tensor([27], device='cuda:0')
            # our type: tensor([[-0.0087,  0.0649,  0.0072, -0.0491, -0.0355,  0.0312,  0.0860,  0.0233,
            #          -0.0190, -0.0398, -0.0222, -0.0094, -0.0119,  0.0175, -0.0238, -0.0178,
            #          -0.0340, -0.0256, -0.0263,  0.0372, -0.0085,  0.0134, -0.0392, -0.0959,
            #          -0.0587, -0.0108, -0.0159,  0.0605,  0.0278, -0.0508]],
            #        device='cuda:0')
            ...
            # plt.colorbar()
            #plt.savefig('results/psdfig/' + str(IDX) + 're.png')
            #plt.clf()
            plt.plot(psd[0].cpu(),lw=2.5)
            plt.plot(recovered_output[0].cpu(), lw=2.5)
            # plt.colorbar()
            plt.savefig('results/psdfig/' + str(IDX) + 'comp.png')
            plt.clf()
            ...
            for x_offset in np.arange(-5, 5, 0.1):
                for y_offset in np.arange(-5, 5, 0.1):
