...
def test_net(rank, other_args):
    ...
    weights = torch.load("//data/vision/torralba/scratch/chuang/Projects/nap_vision/results/gradient/00100.chkpt",
    ...
    with torch.no_grad():
        ...
        for IDX in range(1016):  # 1016
            ...
            # print(residual_pred_logvar.shape)
            ...
            total_latent = torch.cat((type_pred, residual_pred, position_embedded), axis=-1)  # org 1
            ...
            for i in range(30):
                ...
                # position_embedded = position_pred + 0.0
                typei0 = np.zeros(30).astype(np.float32)
                typei0[i] = 1
                typei = torch.tensor(typei0).to(output_device, non_blocking=True)[None]
                # print(type_pred.dtype)
                # print(typei.dtype)
                #total_latent = torch.cat((typei, residual_pred, position_embedded), axis=-1)#position_gt
                total_latent = torch.cat((typei, residual_pred, position_gt), axis=-1)
                recovered_output = decoder(total_latent)

                pos.append(position_pred.cpu().numpy())

                losses.append(reconstruction_criterion(recovered_output, psd).item())
            typepred=np.argmin(np.array(losses))
            second_index = np.argsort(np.array(losses))[-2]
            third_index = np.argsort(np.array(losses))[-3]
            #print(typepred)
            #print("gt:",type_num_gt.cpu().numpy()[0])

            if typepred==type_num_gt.cpu().numpy()[0] or second_index==type_num_gt.cpu().numpy()[0] or third_index==type_num_gt.cpu().numpy()[0]:
                typeright+=1
            print(type_num_gt.cpu().numpy()[0])
            print(typepred)
            print(second_index)
            print(third_index)
            print("type ok:",typeright/(IDX+1))
            #
