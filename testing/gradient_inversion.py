...
def test_net(rank, other_args):
    ...
    weights = torch.load("/data/vision/torralba/scratch/chuang/Projects/nap_vision/results/gradient/00060.chkpt",
   ...
   # encoder.load_state_dict(weights["network"][0])
    ...
    for IDX in range(1000):  # 1016
        ...
        position = torch.randn(1, 2).to(output_device, non_blocking=True)
        type_logit = torch.rand(1, 30).to(output_device, non_blocking=True)
        latent = torch.randn(1, 8).to(output_device, non_blocking=True)
        position.requires_grad = True
        type_logit.requires_grad = True
        latent.requires_grad = True
        optim = torch.optim.Adam([type_logit,latent, position], lr=5e-4)
        #print(type_logit)
        for i in range(20):
            type_final = torch.nn.functional.softmax(type_logit)#.to(output_device, non_blocking=True)
            #print(type_final)
            optim.zero_grad()
            total_latent = torch.cat((type_final, latent, position), axis=-1)
