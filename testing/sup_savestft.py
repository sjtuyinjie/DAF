...
def test_net(rank, other_args):
    ...
    weights = torch.load("//data/vision/torralba/scratch/chuang/Projects/nap_vision/results/sup/00020.chkpt",
    ...
    with torch.no_grad():
        ...
        for IDX in range(1016):  # 1016
            ...
            total_latent = torch.cat((type_pred, residual_pred,position_pred), axis=-1)
            recovered_output = decoder(total_latent)
            inferno_cmap = ListedColormap(plt.cm.get_cmap('inferno', 256).colors)
            plt.imshow(recovered_output[0][0].cpu(),cmap=inferno_cmap)
            #plt.colorbar()

            plt.savefig('results/stftfig2/'+str(IDX)+'releft.png')
            # plt.imshow(recovered_output[0][1].cpu(),cmap=inferno_cmap)
            # #plt.colorbar()
            # plt.savefig('results/stftfig2/' + str(IDX) + 'reright.png')
            plt.imshow(gt[0][0].cpu(),cmap=inferno_cmap)
            #plt.colorbar()

            plt.savefig('results/stftfig2/' + str(IDX) + 'gtleft.png')
            #
            # plt.imshow(gt[0][1].cpu(),cmap=inferno_cmap)
            # #plt.colorbar()
            # plt.savefig('results/stftfig2/' + str(IDX) + 'gtright.png')
