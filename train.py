...
from data_loading.sound_loader_v2 import soundsamples
...
def train_net(rank, world_size, freeport, other_args):
    ...
    time_embedder = embedding_module_log(num_freqs=other_args.num_freqs, ch_dim=2, max_freq=4).to(output_device)
    freq_embedder = embedding_module_log(num_freqs=other_args.num_freqs, ch_dim=2, max_freq=4).to(output_device)
    auditory_net = kernel_residual_fc_embeds(input_ch=44, intermediate_ch=other_args.features, grid_ch=other_args.grid_features, num_block=other_args.layers, grid_gap=other_args.grid_gap, grid_bandwidth=other_args.bandwith_init, bandwidth_min=other_args.min_bandwidth, bandwidth_max=other_args.max_bandwidth, float_amt=other_args.position_float, min_xy=None, max_xy=None,num_objs=30).to(output_device)
    ...
    optimizer = torch.optim.AdamW([
        {'params': grid_container, 'lr': 2e-4, 'weight_decay': 0.0},
        {'params': orig_container, 'lr': 2e-4, 'weight_decay': 0.0}], lr=2e-4, weight_decay=0.0)
    ...
    for epoch in range(start_epoch, other_args.epochs+1):
        ...
        for data_stuff in sound_loader:
            ...
            material_type_num = data_stuff[1].to(output_device, non_blocking=True)[:,0]
            obj_type_num = data_stuff[2].to(output_device, non_blocking=True)[:,0]
            ...
            # if rank ==0:
            #     print(material_type_num, obj_type_num)
            #     exit()
            with torch.no_grad():
                position_embed = non_norm_position.expand(-1, PIXEL_COUNT, -1)
            ...
            try:
                output = ddp_auditory_net(total_in, obj_type_num).squeeze(3).transpose(1, 2)
        ...
        if rank == 0:
            ...
            old_time = time()
        if rank == 0 and (epoch%5==0 or epoch==1 or epoch>(other_args.epochs-3)):
