...
from model.networks_object import FC_layers_3, FC_layers_2,FC_layers_2_mod_16,myresnet
...
def train_net(rank, world_size, freeport, other_args):
    ...
    for epoch in range(start_epoch, other_args.epochs):
        ...
        for data_stuff in sound_loader:
            ...
            #print("shape:",material_pred.shape)#  16 7
            ...
            total_latent = torch.cat((material_final,scale_pred, residual_pred), axis=1)
            recovered_output = decoder_DDP(total_latent)

            # print(scale_num_gt.shape)
            # print(scale_pred.shape)
            ...
            # print("recovered shape:",recovered_output.shape)
            # print("psd shape:",psd.shape)
            loss_reconstruction = reconstruction_criterion(recovered_output, psd)*10#reconstruction psd

            total_loss = loss_material +loss_scale + loss_kld + loss_regularization + loss_reconstruction
            # print(loss_scale)
            # print(loss_material)
        ...
        if rank == 0:
            ...
            print(
                "{}: Ending epoch {}, time {},  Total loss: {:.6}, reconstruction loss: {:.6}, kld loss: {:.6}, material loss: {:.6},scale loss: {:.6}, reg loss: {:.6}".format(
                    other_args.exp_name, epoch, time() - old_time, avg_loss, avg_reconstruction, avg_kld, avg_material,avg_scale, avg_regularization))
            old_time = time()
            f_log = open(basedir, 'a')
            f_log.write("{}: Ending epoch {}, time {},  Total loss: {:.6},  reconstruction loss: {:.6}, kld loss: {:.6}, material loss: {:.6},scale loss: {:.6}, reg loss: {:.6}\n".format(
                    other_args.exp_name, epoch, time() - old_time, avg_loss, avg_reconstruction, avg_kld, avg_material,avg_scale, avg_regularization))
            f_log.flush()
