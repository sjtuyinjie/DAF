...
def train_net(rank, world_size, freeport, other_args):
    ...
    #                        max_type_num=dataset.max_type, max_name_num=dataset.max_name, residual_latent_ch=LATENT_CH).to(output_device)
    decoder = FC_layers_2(input_ch=dataset.max_type + LATENT_CH + 2, intermediate_ch=512,
    ...
    for epoch in range(start_epoch, other_args.epochs):
        ...
        for data_stuff in sound_loader:
            ...
            total_latent = torch.cat((type_pred, residual_pred, position_embedded), axis=1)
            ...
            loss_type = structured_criterion(type_pred, type_num_gt) * TYPE_LOSS_LAMBDA
            #loss_type = 0.0
            #print(position_gt[:,0])
            ...
            loss_regularization = torch.mean(torch.norm(residual_pred, dim=1)) * L2_LAMBDA*0
            loss_reconstruction = reconstruction_criterion(recovered_output, psd)*10#reconstruction psd

            total_loss = loss_material + loss_kld + loss_regularization + loss_reconstruction + loss_position + loss_type#new
        ...
        if rank == 0:
            ...
            print(
                "{}: Ending epoch {}, time {},  Total loss: {:.6}, position loss: {:.6}, reconstruction loss: {:.6}, kld loss: {:.6}, material loss: {:.6}, reg loss: {:.6},type loss: {:.6}".format(
                    other_args.exp_name, epoch, time() - old_time, avg_loss, avg_position, avg_reconstruction, avg_kld, avg_material, avg_regularization,avg_type))
            old_time = time()
            f_log = open(basedir, 'a')
            f_log.write("{}: Ending epoch {}, time {},  Total loss: {:.6}, position loss: {:.6}, reconstruction loss: {:.6}, kld loss: {:.6}, material loss: {:.6}, reg loss: {:.6}, type loss: {:.6}\n".format(
                    other_args.exp_name, epoch, time() - old_time, avg_loss, avg_position, avg_reconstruction, avg_kld, avg_material, avg_regularization,avg_type))
            f_log.flush()
