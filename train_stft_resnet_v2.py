...
from model.networks_stft import FC_layers_3, FC_layers_2,FC_layers_2_mod_16,myresnet
...
def train_net(rank, world_size, freeport, other_args):
    ...
    #                        max_type_num=dataset.max_type, max_name_num=dataset.max_name, residual_latent_ch=LATENT_CH).to(output_device)
    decoder = FC_layers_2(input_ch=dataset.max_material + LATENT_CH + 42, intermediate_ch=512,
                          output_ch=129 + 129, num_blocks=3).to(output_device)
    embedder = embedding_module_log(max_freq=5)
    ...
    # embedder = DDP(embedder, find_unused_parameters=False, device_ids=[rank])
    ...
    for epoch in range(start_epoch, other_args.epochs):
        ...
        for data_stuff in sound_loader:
            ...
            material_pred, type_pred, residual_pred_mu, residual_pred_logvar, position_pred = encoder_DDP(gt)#input spectrogram for encoder


            # print("shape:",material_pred.shape)
            # print("shape2:", material_num_gt.shape)
            ...
            position_embedded = embedder(position_pred)
            # if rank == 0:
            #     print(position_pred.shape, position_embedded.shape, position_embedded.requires_grad, "POSITION INFORMATION")
            # dist.barrier()
            ...
            loss_position = position_criterion(position_pred, position_gt[:,0]) * POS_LAMBDA
            loss_kld = kld(residual_pred_mu, residual_pred_logvar) * KLD_LAMBDA

            loss_reconstruction = reconstruction_criterion(recovered_output, psd)*10#reconstruction psd

            total_loss = loss_material + loss_kld + loss_reconstruction + loss_position
        ...
        if rank == 0:
            ...
            print(
                "{}: Ending epoch {}, time {},  Total loss: {:.6}, position loss: {:.6}, reconstruction loss: {:.6}, kld loss: {:.6}, material loss: {:.6}".format(
                    other_args.exp_name, epoch, time() - old_time, avg_loss, avg_position, avg_reconstruction, avg_kld, avg_material))
            old_time = time()
            f_log = open(basedir, 'a')
            f_log.write("{}: Ending epoch {}, time {},  Total loss: {:.6}, position loss: {:.6}, reconstruction loss: {:.6}, kld loss: {:.6}, material loss: {:.6}\n".format(
                    other_args.exp_name, epoch, time() - old_time, avg_loss, avg_position, avg_reconstruction, avg_kld, avg_material))
            f_log.flush()
        if rank == 0 and (epoch % 10 == 0 or epoch == 1 or epoch > (other_args.epochs - 3)):
            save_name = str(epoch).zfill(5) + "fourier.chkpt"
            save_dict = {}
            print("saved")
